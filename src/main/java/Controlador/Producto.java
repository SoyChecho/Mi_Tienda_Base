	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if(request.getParameter("cargar")!=null) {
        	...
        	if(archivo.getContentType().equals("application/vnd.ms-excel")) {
        		try {
        		InputStream file = archivo.getInputStream(); //Archivo de lectura INputStream
        		File copia = new File(Url+"productos.csv");
        		FileOutputStream escribir = new FileOutputStream(copia);//Archivo de escritura OutputStream
        		int num=file.read();//leyendo los primeros bytes
        		while(num !=-1) {//leer los demas bytes
        			escribir.write(num);//mientras lee vamos escribiendo en le otro achivo
        			num=file.read();
        		}
        		file.close();//cerrar archivo lectura
        		escribir.close();//cerrar archivo escritura
        		JOptionPane.showMessageDialog(null, "Se cargo el archivo correctamente");
        		//Llamar el metodo del DAO
